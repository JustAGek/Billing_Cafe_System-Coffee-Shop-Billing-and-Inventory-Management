...
Class Action {
    ...
    function save_order(){
        ...
        $today_date = date('Y-m-d'); // Change the format to Y-m-d for MySQL DATE type
        $query = $this->db->query("SELECT order_number FROM orders WHERE DATE(date_created) = '$today_date' ORDER BY order_number DESC LIMIT 1");
    
        if($query->num_rows > 0){
            $last_order = $query->fetch_assoc();
            $order_number = $last_order['order_number'] + 1; // Increment last order number
        } else {
            $order_number = 1; // Reset to 1 if no orders today
        }
    
        $data .= ", order_number = '$order_number'"; // Append the order number to data string
    
        // Generate the ref_no
        $ref_no  = mt_rand(1,999999999999);
        $ref_no = sprintf("%'012d", $ref_no);
    
        $data .= ", ref_no = '$ref_no'"; // Append the ref_no to data string
    
        if(empty($id)){
            $save = $this->db->query("INSERT INTO orders SET $data");
            $id = $this->db->insert_id; // Get the new order ID after insertion
        } else {
        ...
        if($save){
            ...
            $ids = implode(',', $ids);
            if(!empty($ids)){
                $this->db->query("DELETE FROM order_items WHERE order_id = $id AND id NOT IN ($ids)");
            }
